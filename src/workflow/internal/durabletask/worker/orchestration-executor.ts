...
export class OrchestrationExecutor {
  ...
  constructor(registry: Registry) {
    ...
    this.logger = new Logger("DurableTask", "OrchestrationExecutor");
  }
  ...
  async execute(
    ...
    try {
      ...
      this.logger.info(`${instanceId}: Processing ${newEvents.length} new history event(s): ${summary}`);
  ...
  private async processEvent(ctx: RuntimeOrchestrationContext, event: pb.HistoryEvent): Promise<void> {
    ...
    // this.logger.debug(`Processing event type ${eventTypeName} (${event.getEventtypeCase()})`);
    ...
    try {
      switch (eventType) {
        ...
        case pb.HistoryEvent.EventtypeCase.EXECUTIONSTARTED:
          {
            ...
            if (isAsyncGenerator) {
              ...
              this.logger.info(`An orchestrator was returned that doesn't schedule any tasks (type = ${resultType})`);
        ...
        case pb.HistoryEvent.EventtypeCase.TIMERFIRED:
          {
            ...
            if (!timerTask) {
              ...
              if (!ctx._isReplaying) {
                this.logger.warn(`${ctx._instanceId}: Ignoring unexpected timerFired event with ID = ${timerId}`);
              }
        ...
        case pb.HistoryEvent.EventtypeCase.TASKCOMPLETED:
          {
            ...
            if (!activityTask) {
              // TODO: Should this be an error? When would it ever happen?
              if (!ctx._isReplaying) {
                this.logger.warn(`${ctx._instanceId}: Ignoring unexpected taskCompleted event with ID = ${taskId}`);
              }
        ...
        case pb.HistoryEvent.EventtypeCase.TASKFAILED:
          {
            ...
            if (!activityTask) {
              // TODO: Should this be an error? When would it ever happen?
              if (!ctx._isReplaying) {
                this.logger.warn(`${ctx._instanceId}: Ignoring unexpected taskFailed event with ID = ${taskId}`);
              }
        ...
        case pb.HistoryEvent.EventtypeCase.EVENTRAISED:
          {
            ...
            if (!ctx._isReplaying) {
              this.logger.info(`${ctx._instanceId}: Event raised: ${eventName}`);
            }
        ...
        case pb.HistoryEvent.EventtypeCase.EXECUTIONSUSPENDED:
          {
            if (!this._isSuspended && !ctx._isReplaying) {
              this.logger.info(`${ctx._instanceId}: Execution suspended`);
            }
        ...
        case pb.HistoryEvent.EventtypeCase.EXECUTIONTERMINATED: {
          if (!ctx._isReplaying) {
            this.logger.info(`${ctx._instanceId}: Execution terminated`);
          }
        ...
        default:
          this.logger.info(`Unknown history event type: ${eventTypeName} (value: ${eventType}), skipping...`);
      ...
      this.logger.error(`Could not process the event ${eventTypeName} due to error ${e}`);
      throw e;
    }
  }
}
