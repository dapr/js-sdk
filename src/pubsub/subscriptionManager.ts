...
export class SubscriptionManager {
     ...
     */
    public addEventHandlerToSubscription(pubsub: string, topic: string, handler: TypeDaprPubSubCallback, route?: string): void {
        const routeName = this.getSanitizedRouteWithDefault(route);
        const subscription = this.getSubscription(pubsub, topic);
        if (!subscription) {
            throw new Error(`The topic '${topic}' is not subscribed to PubSub '${pubsub}',`
    ...
    private getRouteFromPath(pubsub: string, topic: string, path: string): string {
        ...
        return path.replace(`${pubsub}--${topic}--`, "").replace("/", "");
    }
     ...
     */
    private getSanitizedRouteWithDefault(route?: string, defaultRoute: string = Settings.getDefaultPubSubRouteName()): string {
    ...
    private generatePubSubPath(pubsub: string, topic: string, route?: string): string {
        let routeName = this.getSanitizedRouteWithDefault(route);
        return `${pubsub.toLowerCase()}--${topic.toLowerCase()}--${routeName}`;
    }
     ...
     */
    private generateRoutes(pubsub: string, topic: string, options: PubSubSubscriptionOptionsType = {}): PubSubSubscriptionTopicRoutesType {
        ...
        if (options.deadLetterTopic || options.deadLetterCallback) {
            const routeName = this.getSanitizedRouteWithDefault(
                options.deadLetterTopic, Settings.getDefaultPubSubDeadLetterRouteName());
